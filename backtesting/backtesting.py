...
class _Broker:
    def __init__(self, *, data, cash, commission, margin,
                 trade_on_close, hedging, exclusive_orders, index, spread):
    ...
    def _adjusted_price(self, size=None, price=None) -> float:
        """
        Adjust the price for the spread.
        Long positions have a higher entry price (ask), short positions have a lower entry price (bid).
        """
        base_price = price or self.last_price
        if size is not None:
            if size > 0:
            ...
            if need_size:
                ...
                if order.sl or order.tp:
                    if is_market_order:
                        ...
                        warnings.warn(
                            f"({data.index[-1]}) A contingent SL/TP order was not executed "
                            f"due to the exclusive_orders mode and the next() order of execution.")
    ...
    def _reduce_trade(self, trade: Trade, price: float, size: float, time_index: int):
        ...
        assert abs(size) <= abs(trade.size)

        if abs(size) < abs(trade.size):
            new_size = trade.size + size
            if not self._hedging:
                for t in self.trades:
                    if t == trade:
                        continue
                    assert t.size * new_size >= 0
            trade._size = new_size
        else:
            self.trades.remove(trade)
            self.closed_trades.append(trade)
        self._cash -= size * price / self._leverage
        self._cash -= abs(size) * self._spread / 2  # Adjust for spread on trade reduction

    def _close_trade(self, trade: Trade, price: float, time_index: int):
        self._cash -= trade.size * price / self._leverage
        self._cash -= abs(trade.size) * self._spread / 2  # Adjust for spread on trade closing
        self.trades.remove(trade)
        self.closed_trades.append(trade)
        trade._close(price, time_index)
    ...
    def _open_trade(self, price: float, size: int,
        ...
        assert size == round(size)
        self._cash -= size * price / self._leverage
        self._cash -= abs(size) * self._spread / 2  # Adjust for spread on trade opening
        trade = Trade(self, price, size, time_index, tag)
        self.trades.append(trade)

        if sl:
            sl = sl * (1 - self._spread / 2) if size > 0 else sl * (1 + self._spread / 2)
            trade._sl_order = self.new_order(-size, stop=sl, trade=trade)
        if tp:
            tp = tp * (1 + self._spread / 2) if size > 0 else tp * (1 - self._spread / 2)
            trade._tp_order = self.new_order(-size, stop=tp, trade=trade)
